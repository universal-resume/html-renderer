type ProfilesArgs = Array<{
	network: string;
	username?: string | undefined;
	url: string;
}>;

function Profiles(profiles: ProfilesArgs) {
	const logos = {
	...
	return {
		build: () => {
			...
			ul.classList.add("gap-3", "py-4", "flex", "flex-col");
			for (const profile of profiles) {
				const li = document.createElement("li");
				li.classList.add("text-white");
				const logo = logos[profile.network] || "";
				li.innerHTML = `
                    <a href="${profile.url}" class="font-bold text-sm flex items-center justify-center gap-1">
                        ${logo ? logo : ""} ${profile.network}
                            </a>
                            `;
				li.innerHTML += `<a href="${profile.url}" class="flex items-center justify-center text-xs">

...
export const Location = (location: string | undefined, { color }: Theme): string => {
    if (!location) return '';
    return `
        <div class="text-xs flex items-center gap-1 text-${color}-600 hover:text-${color}-700">
            ...
            <a target="_blank" href="https://www.google.com/search?q=${location}" class="hover:underline">${location}</a>
        </div>
    `
}
...
export function Sidebar({ color }: Theme): Sidebar {
    ...
    pageHeightContainer.classList.add('page-sidebar', 'w-1/4', 'min-h-full', `bg-${color}-900`, 'p-4');
    ...
    function pushItem(item: HTMLElement) {
        return new Promise<{ height: number, limit: number }>((resolve, reject) => {
            ...
            requestAnimationFrame(() => {
                if (dynamicHeightContainer.offsetHeight > dynamicContainerMaxHeight && dynamicHeightContainer.lastChild) {

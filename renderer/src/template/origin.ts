...
export async function origin(resume: Resume.Type, theme: Theme, { addPage, getPages }: RendererActions) {
    ...
    function generateSectionTitle({ title, icon }: { title: string, icon: string }, { color }: Theme) {
        const sectionTitle = document.createElement('h2');
        sectionTitle.setAttribute('id', `section-title-${title.toLowerCase().replace(/ /g, '-')}`);
        sectionTitle.classList.add('text-md', 'pb-1', 'font-semibold', 'flex', 'items-center', 'gap-2', `text-${color}-800`);
    ...
    if (resume.basics.summary) {
        await addSidebarItem(Summary({
            text: resume.basics.summary
        }).build())
    }

    const skills = (resume.skills || []).map(skill => skill.tags).flat().filter(tag => tag !== undefined)
    if (skills.length > 0) {
    ...
    if (resume.basics.location && resume.basics.location.city) {
        await addSidebarItem(Location({ city: resume.basics.location.city, remote: resume.basics.location.remote }).build())
    }

    if (resume.basics.driverLicenses && resume.basics.driverLicenses.length > 0) {
        await addSidebarItem(DriverLicenses([...resume.basics.driverLicenses]).build())
    }

    if (resume.basics.phone || resume.basics.email) {
        await addSidebarItem(Contact({ phone: resume.basics.phone, email: resume.basics.email }).build())
    }
